...
# Filter for Boise
boise_df = df[df['Location'] == 'Boise, ID'].copy()

# Experience groups
boise_df['Experience Group'] = boise_df['Overall Experience'].apply(
    lambda x: '1-3 Stars' if x <= 3 else '4-5 Stars'
)

# Clean eating time
boise_df['Eating Time'] = boise_df['Eating Time'].astype(str).replace('', 'Unknown')
eating_order = ['Breakfast', 'Brunch', 'Lunch', 'Dinner', 'Take-Out']

# ========== 1. Average Ratings ==========
avg_ratings = boise_df[[
...
plt.title('Average Ratings Overview - Boise')
plt.ylabel('Average Rating')
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig("average_ratings_boise.png", bbox_inches="tight")
plt.show()
...
rating_distribution = pd.DataFrame({
    col: boise_df[col].value_counts().sort_index() for col in rating_columns
}).fillna(0).astype(int).T

rating_distribution.plot(kind='bar', stacked=True, figsize=(10, 6), colormap='coolwarm')
plt.title('Rating Distributions by Category - Boise')
plt.xlabel('Rating Category')
plt.ylabel('Number of Ratings')
plt.legend(title='Stars', bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig("rating_distribution_boise.png", bbox_inches="tight")
plt.show()

# ========== 3. Stacked Bar by Eating Time ==========
eating_time_counts = pd.crosstab(boise_df['Eating Time'], boise_df['Experience Group']).reindex(eating_order)

eating_time_counts.plot(kind='bar', stacked=True, color=['#FF9999', '#66C2A5'], figsize=(8, 6))
plt.title('Overall Experience by Eating Time (Stacked) - Boise')
plt.xlabel('Eating Time')
plt.ylabel('Number of Feedbacks')
plt.xticks(rotation=45)
plt.legend(title='Rating Group')
plt.tight_layout()
plt.savefig("eating_time_experience_boise.png", bbox_inches="tight")
plt.show()

# ========== 4. Histogram - Party Size vs Experience ==========
plt.figure(figsize=(8, 5))
sns.histplot(data=boise_df, x='Overall Experience', hue='Party Size', multiple='dodge', shrink=0.8, bins=5)
plt.title('Experience Rating Distribution by Party Size - Boise')
plt.xlabel('Overall Experience')
plt.ylabel('Count')
plt.tight_layout()
plt.savefig("party_size_experience_boise.png", bbox_inches="tight")
plt.show()

# ========== 5. Histogram - Price Rating vs Experience ==========
plt.figure(figsize=(8, 5))
sns.histplot(data=boise_df, x='Overall Experience', hue='Price Rating', multiple='dodge', shrink=0.8, bins=5)
plt.title('Experience Rating Distribution by Price Rating - Boise')
plt.xlabel('Overall Experience')
plt.ylabel('Count')
plt.tight_layout()
plt.savefig("price_rating_experience_boise.png", bbox_inches="tight")
plt.show()